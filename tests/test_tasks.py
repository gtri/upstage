...
class KnowTask(Task):
    ...
    def task(self, *, actor: Actor) -> TASK_GEN:
        ...
        know = self.get_actor_bulk_knowledge(
            actor,
            ("thing1", "thing2"),
            must_exist=True,
        )
        assert know == {"thing1": "Hello", "thing2": 6.28}
        yield Wait(actor.time)
...
def _task_runner(env: Environment, rate: float, timeout_point: float, final: bool=False) -> SIMPY_GEN:
    ...
    if task_generator.is_alive:
        task_object._final_interrupt = final
...
def test_task() -> None:
    with EnvironmentContext() as env:
        ...
        t.run(actor=a)
        env.run()
        assert env.now == a.time
        thelog = a.get_log()
        assert len(thelog) == 2
        assert "Reason: KnowTask" in thelog[0][1]
        assert "Reason: KnowTask" in thelog[1][1]
    ...
    with EnvironmentContext():
        ...
        with EnvironmentContext() as env:
            with pytest.raises(SimulationError):
                task_instance = BrokenTask(BlankEvent)
                task_instance.run(actor=actor)
                env.run()
...
def test_interrupting_final() -> None:
    with EnvironmentContext() as env:
        timeout_point = 0.5
        rate = 3.5
        proc = env.process(
...
class Restartable(Task):
    ...
    def on_interrupt(self, *, actor: Dummy, cause: Any) -> InterruptStates:
        if cause == "restart":
            return InterruptStates.RESTART
        else:
...
def test_terminal_task_run(
    ...
    class EndPoint(TerminalTask):
        def log_message(self, *, actor: Actor) -> str:
            return "The Message"

    class EndPointBase(TerminalTask):
        pass
    ...
    with EnvironmentContext() as env:
        ...
        assert "The Message" in actor._log[-1][1]
        ...
        assert "Entering terminal task:" in actor._log[-1][1]
