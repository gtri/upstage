...
def test_put_event_with_stores() -> None:
    with EnvironmentContext() as env:
        ...
        assert put_event.rehearsal_time_to_complete == 0.0, "Incorrect time to complete"
...
def test_get_event_with_stores() -> None:
    with EnvironmentContext() as env:
        ...
        event = Get(store)
        assert event.rehearsal_time_to_complete == 0.0, "Incorrect time to complete"
        ...
        env.run()
        assert isinstance(event._simpy_event, StoreGet)
        item = event._simpy_event.value
...
def test_get_event_with_containers() -> None:
    with EnvironmentContext() as env:
        ...
        env.run()
        assert isinstance(event._simpy_event, ContainerGet)
        amount = event._simpy_event.amount
        assert amount == get_arg, "Returned item is not the original item"
        assert amount == event.get_value()
...
def test_resource_events() -> None:
    with EnvironmentContext() as env:
        ...
        assert a_resource.users[0] is request_object._simpy_event, "The user is the request object"
        ...
        assert new_request._simpy_event is not None
        assert not new_request._simpy_event.processed, "Request went through when it shouldn't"

        # put the old one back
        request_object.as_event()
        env.run()
        assert new_request._simpy_event is not None
        assert new_request._simpy_event.processed, "Follow-on request didn't go through"
        ...
        assert newest_request._simpy_event is not None
        assert not newest_request._simpy_event.processed, "Request went through when it shouldn't"

        assert newest_request._simpy_event in a_resource.put_queue, (
        ...
        env.run()
        assert newest_request._simpy_event not in a_resource.put_queue, (
...
def test_basic_usage() -> None:
    with EnvironmentContext() as env:
        event = Event()
        assert event._simpy_event is not None
        assert isinstance(event._simpy_event, SIM.Event)
        assert event._simpy_event is event.as_event()
        ...
        assert event.rehearsal_time_to_complete == 0.0

        last_event = event._simpy_event
        event.reset()
        assert last_event is not event._simpy_event
...
def test_filter_store_events() -> None:
    with EnvironmentContext() as env:
        store1 = SIM.FilterStore(env)

        def _put_stuff() -> SIMPY_GEN:
            yield Wait(1.0).as_event()
            yield store1.put("thing")
            yield Wait(1.0).as_event()
            yield store1.put("other")

        def _get_stuff() -> SIMPY_GEN:
            g1 = FilterGet(store1, filter=lambda item: "oth" in item)
            yield g1.as_event()
            assert g1.get_value() == "other"
            assert "thing" in store1.items
