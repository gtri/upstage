...
4. ``init``: If ``False``, keep the state out of the Actor's init, and raise an error if it's input.
...
The ``init=False`` setting requires a default to be set. The purpose of this setting is to hint to the user that
...
receives an input on initialization, an error will be thrown.
